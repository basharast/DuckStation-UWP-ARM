...
extern std::string GetFolderParent(const std::string& str);
extern std::string GetFolderName(const std::string& str);
extern std::string resolvePath(std::string& itemName);
extern bool IsValidUWP(std::string path, bool allowForAppData);
extern bool dataFileExists(std::string path);
extern bool isDataFile(std::string path);
...
public:
  ...
  virtual ~AtomicUpdatedFileByteStream()
  {
    if (m_discarded)
    {
      ...
      // delete the temporary file
      // fclose(m_pFile);
      if (!DeleteFileWPtr(StringUtil::UTF8StringToWideString(m_temporaryFileName).c_str()))
      {
        if (IsValidUWP(m_temporaryFileName, false))
        {
          try
          {
            std::string str = m_temporaryFileName;
            resolvePath(str);
...
GrowableMemoryByteStream::GrowableMemoryByteStream(void* pInitialMem, u32 InitialMemSize)
{
  ...
  Log_WarningPrintf("GrowableMemoryByteStream: InitialMemSize = %d", InitialMemSize);
  ...
  else
  {
    ...
    // m_pPrivateMemory = m_pMemory =(u8*) VirtualAllocPtr(nullptr, m_iMemorySize, MEM_RESERVE, PAGE_READWRITE);
  }
}
...
GrowableMemoryByteStream::~GrowableMemoryByteStream()
{
  ...
  /*if (m_pPrivateMemory != nullptr)
    VirtualFree(m_pPrivateMemory, 0, MEM_RELEASE);*/
}
...
{
  Log_WarningPrintf("ResizeMemory: current = %d, new = %d", m_iMemorySize, new_size);
  if (new_size == m_iMemorySize)
    return;

  if (m_pPrivateMemory == nullptr)
  {
    Log_WarningPrintf("ResizeMemory: Initial");
  ...
  {
    Log_WarningPrintf("ResizeMemory: realloc");
...
std::unique_ptr<ByteStream> ByteStream::OpenFile(const char* fileName, u32 openMode)
{
  ...
  if (openMode & BYTESTREAM_OPEN_ATOMIC_UPDATE)
  {
...
#else
    auto share = FILE_SHARE_DELETE;
    HANDLE hFile = CreateFile2Ptr(wideTemporaryFileName.c_str(), desiredAccess, share, CREATE_NEW, nullptr);
#endif

    auto fileMode = GetFileMode(std::string(modeString).c_str());
    if (hFile == INVALID_HANDLE_VALUE)
    {
      winrt::Windows::Storage::StorageFile testFile = nullptr;
      if (IsValidUWP(convert(wideTemporaryFileName), false))
      {
        std::string str = convert(wideTemporaryFileName);

        try
        {
          bool createIfNotExists = true; // CREATE_NEW
          ...
          try
          {
            ...
            if (testFile != nullptr)
            {
              ...
              {
                GetFileHandle(testFile, &hFile, GetAccessMode(desiredAccess), GetShareMode(share));
              }
            }
          }
          catch (...)
          ...
          if (testFile == nullptr)
          {
            if (createIfNotExists)
            {
              ...
              {
                std::wstring parent =
                  StringUtil::UTF8StringToWideString(GetFolderParent(convert(wideTemporaryFileName)));
                std::wstring name = StringUtil::UTF8StringToWideString(GetFolderName(convert(wideTemporaryFileName)));
                auto testFolder =
                ...
                if (testFolder != nullptr)
                {
                  ...
                  {
                    GetFileHandle(createTest, &hFile, GetAccessMode(desiredAccess), GetShareMode(share));
                  }
                }
              }
              catch (...)
    ...
    {
      CloseHandle(hFile);
      FileSystem::DeleteFile(convert(wideTemporaryFileName).c_str());
      // DeleteFileWPtr(wideTemporaryFileName.c_str());
      return nullptr;
    }
    ...
    std::unique_ptr<AtomicUpdatedFileByteStream> pStream =
      std::make_unique<AtomicUpdatedFileByteStream>(pTemporaryFile, std::string(fileName), temporaryFileName);
