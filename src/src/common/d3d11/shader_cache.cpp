...
bool ShaderCache::CreateNew(const std::string& index_filename, const std::string& blob_filename)
{
  if (!cacheState)
  {
    Log_WarningPrint("Create shader cache ignored due to settings");
    return false;
  }
...
bool ShaderCache::ReadExisting(const std::string& index_filename, const std::string& blob_filename)
{
  if (!cacheState)
  {
    Log_WarningPrint("Shader cache ignored due to settings");
    return false;
  }
                                                                   ...
                                                                   std::string_view shader_code)
{
  ...
  if (cacheState)
  {
    ...
    auto a1 = std::fwrite(blob->GetBufferPointer(), 1, entry.blob_size, m_blob_file);
    auto a2 = std::fflush(m_blob_file);
    auto a3 = std::fwrite(&entry, sizeof(entry), 1, m_index_file);
    auto a4 = std::fflush(m_index_file);
    if (a1 != entry.blob_size || a2 != 0 || a3 != 1 || a4 != 0)
    {
      Log_ErrorPrintf("fflush() failed: %08X", GetLastError());
  ...
  {
    Log_WarningPrint("Save shader cache ignored due to settings");
  }
