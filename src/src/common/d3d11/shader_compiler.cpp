...
ComPtr<ID3DBlob> CompileShader(Type type, D3D_FEATURE_LEVEL feature_level, std::string_view code, bool debug)
{
  ...
  switch (feature_level)
  {
    ...
    {
      static constexpr std::array<const char*, 4> targets = {
        {"vs_4_0_level_9_1", nullptr, "ps_4_0_level_9_1", nullptr}};
    ...
    {
      static constexpr std::array<const char*, 4> targets = {
        {"vs_4_0_level_9_3", nullptr, "ps_4_0_level_9_3", nullptr}};
      target = targets[static_cast<int>(type)];
    }
    break;
    ...
    {
      static constexpr std::array<const char*, 4> targets = {{"vs_4_0", "gs_4_0", "ps_5_0", "cs_4_0"}};
      target = targets[static_cast<int>(type)];
    }
    break;
  ...
  static constexpr UINT flags_non_debug2 = D3DCOMPILE_OPTIMIZATION_LEVEL2;
  ...
  switch (shader_opt)
  {
    case 0:
      hr =
        D3DCompile(code.data(), code.size(), "0", nullptr, nullptr, "main", target,
                   isLevel93 ? flags_non_debug2 : flags_non_debug, 0, blob.GetAddressOf(), error_blob.GetAddressOf());
        break;
        
    case 1:
        hr =
          D3DCompile(code.data(), code.size(), "0", nullptr, nullptr, "main", target, D3DCOMPILE_OPTIMIZATION_LEVEL3,
                        0, blob.GetAddressOf(), error_blob.GetAddressOf());
        break;
        
    case 2:
        hr = D3DCompile(code.data(), code.size(), "0", nullptr, nullptr, "main", target, D3DCOMPILE_OPTIMIZATION_LEVEL2,
                        0, blob.GetAddressOf(), error_blob.GetAddressOf());
        break;
        
    case 3:
        hr = D3DCompile(code.data(), code.size(), "0", nullptr, nullptr, "main", target, D3DCOMPILE_OPTIMIZATION_LEVEL1,
                        0, blob.GetAddressOf(), error_blob.GetAddressOf());
        break;
        
    case 4:
        hr = D3DCompile(code.data(), code.size(), "0", nullptr, nullptr, "main", target, D3DCOMPILE_OPTIMIZATION_LEVEL0,
                        0, blob.GetAddressOf(), error_blob.GetAddressOf());
        break;
  }
  ...
  if (FAILED(hr))
  {
    Log_ErrorPrintf("Failed to compile '%s':\n%s\n\ncode:\n%s", target, error_string.c_str(), code.data());
