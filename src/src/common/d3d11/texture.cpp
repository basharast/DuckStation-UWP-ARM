                     ...
                     bool dynamic)
{
  ...
  if (FAILED(tex_hr))
  {
    auto hrr = device->GetDeviceRemovedReason();
    Log_ErrorPrintf(
      "Create texture failed: 0x%08X (%ux%u levels:%u samples:%u format:%u bind_flags:%X initial_data:%p), reason: 0x%08X", tex_hr, width, height, levels, samples, static_cast<unsigned>(format), bind_flags, initial_data,
                    hrr);
    return false;
  }
