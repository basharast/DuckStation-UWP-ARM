...
bool StreamBuffer::Create(ID3D11Device* device, D3D11_BIND_FLAG bind_flags, u32 size)
{
  ...
  desc.Usage = D3D11_USAGE_DYNAMIC;             // Usage
  desc.ByteWidth = size;                        // Size of the buffer
  desc.BindFlags = bind_flags;                  // Bind flag
  desc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE; // CPU access flag
...
StreamBuffer::MappingResult StreamBuffer::Map(ID3D11DeviceContext* context, u32 alignment, u32 min_size)
{
  ...
  try
  {
    //if (!appSuspended)
    {
      ...
      if (FAILED(hr))
      {
        Log_ErrorPrintf("Map failed: 0x%08X (alignment %u, minsize %u, size %u, position %u, map type %u)", hr,
                        alignment, min_size, m_size, m_position, static_cast<u32>(map_type));
        Panic("Map failed");
        return {};
      }
    }
  ...
  {
    Panic("MappingResult StreamBuffer::Map failed");
    return {};
  }
