...
extern SIZE_T(WINAPI* VirtualQueryPtr)(LPCVOID, PMEMORY_BASIC_INFORMATION, SIZE_T);
extern BOOL(WINAPI* FlushInstructionCachePtr)(HANDLE, LPCVOID, SIZE_T);
...
bool JitCodeBuffer::Allocate(u32 size /* = 64 * 1024 * 1024 */, u32 far_code_size /* = 0 */)
{
...
#if defined(_WIN32)
  if (g_settings.safe_memory)
  {
#if _M_ARM || defined(_M_ARM64) 
    m_code_ptr = static_cast<u8*>(
      VirtualAllocFromApp(GetCurrentProcess(), m_total_size, MEM_COMMIT, PAGE_READWRITE));
    if (m_code_ptr)
    {
      ULONG old_protection;
      if (!VirtualProtectPtr(m_code_ptr, m_total_size, PAGE_EXECUTE_READWRITE, &old_protection))
      {
  ...
  {
    m_code_ptr = static_cast<u8*>(VirtualAllocPtr(nullptr, m_total_size, MEM_COMMIT, PAGE_EXECUTE_READWRITE));
  }
...
bool JitCodeBuffer::Initialize(void* buffer, u32 size, u32 far_code_size /* = 0 */, u32 guard_size /* = 0 */)
{
...
#if defined(_WIN32)
  DWORD old_protect = 0;
  if (!VirtualProtectPtr(buffer, size, PAGE_EXECUTE_READWRITE, &old_protect))
  {
  ...
  if (guard_size > 0)
  {
    ...
    if (!VirtualProtectPtr(buffer, guard_size, PAGE_NOACCESS, &old_guard_protect) ||
        !VirtualProtectPtr(guard_at_end, guard_size, PAGE_NOACCESS, &old_guard_protect))
    {
...
#if defined(_WIN32)
    DWORD old_protect = 0;
    if (!VirtualProtectPtr(m_code_ptr, m_total_size, m_old_protection, &old_protect))
...
{
#if defined(_WIN32)
  ::FlushInstructionCachePtr(GetCurrentProcess(), address, size);
