...
void* MemoryArena::FindBaseAddressForMapping(size_t size)
{
  void* base_address;
#if defined(_WIN32)
  base_address = VirtualAllocPtr(nullptr, size, MEM_RESERVE, PAGE_READWRITE);
  if (base_address)
...
bool MemoryArena::Create(size_t size, bool writable, bool executable)
{
  ...
  {
    m_file_handle =
      CreateFileMappingWPtr(INVALID_HANDLE_VALUE, nullptr, protect, Truncate32(size >> 32), Truncate32(size),
                            StringUtil::UTF8StringToWideString(file_mapping_name).c_str());
  }
...
bool MemoryArena::ReleaseViewPtr(void* address, size_t size)
{
  bool result;
#if defined(_WIN32)
  result = static_cast<bool>(UnmapViewOfFilePtr(address));
#elif defined(__linux__) || defined(__APPLE__) || defined(__FreeBSD__)
...
void* MemoryArena::CreateReservedPtr(size_t size, void* fixed_address /*= nullptr*/)
{
  ...
  if (g_settings.safe_memory)
  {
    base_pointer = VirtualAllocFromApp(fixed_address, size, MEM_RESERVE, PAGE_NOACCESS);
  }
  else
  {
    base_pointer = VirtualAllocPtr(fixed_address, size, MEM_RESERVE, PAGE_NOACCESS);
  }
#elif defined(__linux__) || defined(__APPLE__) || defined(__FreeBSD__)
  const int flags =
...
bool MemoryArena::SetPageProtection(void* address, size_t length, bool readable, bool writable, bool executable)
{
  ...
  if (g_settings.safe_memory)
  {
    return static_cast<bool>(
      VirtualProtectFromApp(address, length, protection_table[readable][writable][executable], &old_protect));
  }
  else
  {
    return static_cast<bool>(
      VirtualProtectPtr(address, length, protection_table[readable][writable][executable], &old_protect));
  }
