...
static bool InstallFunctionTableCallback(const void* owner, void* start_pc, u32 code_size)
{
  ...
  if (!VirtualProtectPtr(uh, UNWIND_HANDLER_ALLOC_SIZE, PAGE_EXECUTE_READWRITE, &old_protection))
  {
  ...
  uh->runtime_function.FunctionLength = code_size;
  uh->runtime_function.UnwindData = offsetof(UnwindHandler, unwind_info);
#else
