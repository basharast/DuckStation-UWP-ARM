...
protected:
  ...
  if (isLevel93)
    {
      return m_batch_current_vertex_ptr2 == m_batch_start_vertex_ptr2;
  }
    else
    {
  ...
  if (isLevel93)
  {
      return static_cast<u32>(m_batch_end_vertex_ptr2 - m_batch_current_vertex_ptr2);
  }
  else
  {
  ...
  if (isLevel93)
  {
      return static_cast<u32>(m_batch_current_vertex_ptr2 - m_batch_start_vertex_ptr2);
  }
  else
  {
  ...
  static void HandleFlippedQuadTextureCoordinates2(BatchVertex2* vertices);

  /// Computes polygon U/V boundaries.
  static void ComputePolygonUVLimits(BatchVertex* vertices, u32 num_vertices);
  static void ComputePolygonUVLimits2(BatchVertex2* vertices, u32 num_vertices);
  ...
  void CheckForDepthClear2(const BatchVertex2* vertices, u32 num_vertices);
  ...
  BatchVertex2* m_batch_start_vertex_ptr2 = nullptr;
  BatchVertex2* m_batch_end_vertex_ptr2 = nullptr;
  BatchVertex2* m_batch_current_vertex_ptr2 = nullptr;
...
private:
  ...
  enum : u32
  {
    MIN_BATCH_VERTEX_COUNT2 = 6,
    MAX_BATCH_VERTEX_COUNT2 = VERTEX_BUFFER_SIZE / sizeof(BatchVertex2)
  };
  ...
  ALWAYS_INLINE void AddVertex2(const BatchVertex2& v)
  {
    std::memcpy(m_batch_current_vertex_ptr2, &v, sizeof(BatchVertex2));
    m_batch_current_vertex_ptr2++;
  }
  ...
  template<typename... Args>
  ALWAYS_INLINE void AddNewVertex2(Args&&... args)
  {
    m_batch_current_vertex_ptr2->Set(std::forward<Args>(args)...);
    m_batch_current_vertex_ptr2++;
  }
