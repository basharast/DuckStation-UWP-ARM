...
void GPU_HW::HandleFlippedQuadTextureCoordinates2(BatchVertex2* vertices)
{
  // Taken from beetle-psx gpu_polygon.cpp
  // For X/Y flipped 2D sprites, PSX games rely on a very specific rasterization behavior. If U or V is decreasing in X
  // or Y, and we use the provided U/V as is, we will sample the wrong texel as interpolation covers an entire pixel,
...
void GPU_HW::ComputePolygonUVLimits2(BatchVertex2* vertices, u32 num_vertices)
{
  u16 min_u = vertices[0].u, max_u = vertices[0].u, min_v = vertices[0].v, max_v = vertices[0].v;
  for (u32 i = 1; i < num_vertices; i++)
  {
...
void GPU_HW::CheckForDepthClear2(const BatchVertex2* vertices, u32 num_vertices)
{
  DebugAssert(num_vertices == 3 || num_vertices == 4);
  float average_z;
  if (num_vertices == 3)
...
void GPU_HW::DrawLine(float x0, float y0, u32 col0, float x1, float y1, u32 col1, float depth)
{
  ...
  if (isLevel93)
  {
    std::array<BatchVertex2, 4> output;
    if (dx == 0.0f && dy == 0.0f)
    {
  ...
  switch (rc.primitive)
  {
    case GPUPrimitive::Polygon:
    {
      ...
      if (isLevel93)
      {
        ...
        std::array<BatchVertex2, 4> vertices;
        ...
        if (pgxp)
        {
          ...
          else if (g_settings.gpu_pgxp_depth_buffer)
          {
            ...
            if (use_depth)
              CheckForDepthClear2(vertices.data(), num_vertices);
          }
        }

        if (rc.quad_polygon && m_resolution_scale > 1)
          HandleFlippedQuadTextureCoordinates2(vertices.data());

        if (m_using_uv_limits && textured)
          ComputePolygonUVLimits2(vertices.data(), num_vertices);
        ...
        else
        {
          ...
          std::memcpy(m_batch_current_vertex_ptr2, vertices.data(), sizeof(BatchVertex2) * 3);
          m_batch_current_vertex_ptr2 += 3;
        }
        ...
        if (rc.quad_polygon)
        {
          ...
          if ((max_x_123 - min_x_123) >= MAX_PRIMITIVE_WIDTH || (max_y_123 - min_y_123) >= MAX_PRIMITIVE_HEIGHT)
          {
            Log_DebugPrintf("Culling too-large polygon (quad second half): %d,%d %d,%d %d,%d",
                            ...
                            native_vertex_positions[1][1], native_vertex_positions[0][0],
                            native_vertex_positions[0][1]);
          }
          else
          {
            const u32 clip_left =
              static_cast<u32>(std::clamp<s32>(min_x_123, m_drawing_area.left, m_drawing_area.right));
            const u32 clip_right =
              static_cast<u32>(std::clamp<s32>(max_x_123, m_drawing_area.left, m_drawing_area.right)) + 1u;
            const u32 clip_top =
              static_cast<u32>(std::clamp<s32>(min_y_123, m_drawing_area.top, m_drawing_area.bottom));
            const u32 clip_bottom =
            ...
            AddVertex2(vertices[2]);
            AddVertex2(vertices[1]);
            AddVertex2(vertices[3]);
          }
        }
    ...
    case GPUPrimitive::Rectangle:
    {
      ...
      for (s32 y_offset = 0; y_offset < rectangle_height;)
      {
        ...
        for (s32 x_offset = 0; x_offset < rectangle_width;)
        {
          ...
          if (isLevel93)
          {
            const float uv_limits = BatchVertex2::PackUVLimits(tex_left, tex_right - 1, tex_top, tex_bottom - 1);

            AddNewVertex2(quad_start_x, quad_start_y, depth, 1.0f, color, texpage, tex_left, tex_top, uv_limits);
            AddNewVertex2(quad_end_x, quad_start_y, depth, 1.0f, color, texpage, tex_right, tex_top, uv_limits);
            AddNewVertex2(quad_start_x, quad_end_y, depth, 1.0f, color, texpage, tex_left, tex_bottom,
                          uv_limits);

            AddNewVertex2(quad_start_x, quad_end_y, depth, 1.0f, color, texpage, tex_left, tex_bottom,
                          uv_limits);
            AddNewVertex2(quad_end_x, quad_start_y, depth, 1.0f, color, texpage, tex_right, tex_top, uv_limits);
            AddNewVertex2(quad_end_x, quad_end_y, depth, 1.0f, color, texpage, tex_right, tex_bottom, uv_limits);
          }
          else
          {
...
void GPU_HW::EnsureVertexBufferSpace(u32 required_vertices)
{
  if (isLevel93)
  {
    if (m_batch_current_vertex_ptr2)
    {
...
void GPU_HW::EnsureVertexBufferSpaceForCurrentCommand()
{
  ...
  if (isLevel93)
  {
    ...
    else if (m_batch_current_vertex_ptr2)
    {
      if (GetBatchVertexSpace() >= required_vertices)
        return;
