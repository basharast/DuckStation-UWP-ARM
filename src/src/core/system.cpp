...
extern bool force_software_renderer_global;
#ifdef WITH_CHEEVOS
#include "cheevos.h"
#endif
...
bool Initialize(bool force_software_renderer)
{
  ...
  if (!CreateGPU(force_software_renderer || force_software_renderer_global ? GPURenderer::Software : g_settings.gpu_renderer))
  {
    Bus::Shutdown();
    CPU::Shutdown();
    return false;
...
void RunFrames()
{
  ...
  const u32 max_frames_to_run = g_settings.max_frames_to_run;
  u32 frames_run = 0;
