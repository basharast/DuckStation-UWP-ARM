...
{
  int r = int(roundEven(v.r * 31.0));
  int g = int(roundEven(v.g * 31.0));
  int b = int(roundEven(v.b * 31.0));
  int a = (v.a != 0.0) ? 1 : 0;
  return uint(abs(r + (g * 32) + (b * 1024) + (a * 32768)));
}
    ...
    if (isLevel93)
    {
      if (m_uv_limits)
      {
        DeclareVertexEntryPoint(
          ss, {"float4 a_pos", "float4 a_col0", "float a_texcoord", "float a_texpage", "float4 a_uv_limits"}, 1, 1,
          {{"", "float4 v_texpage"}, {"", "float4 v_uv_limits"}}, false, "", UsingMSAA(),
          UsingPerSampleShading());
      }
      else
      {
        DeclareVertexEntryPoint(ss, {"float4 a_pos", "float4 a_col0", "float a_texcoord", "float a_texpage"}, 1, 1,
                                {{"", "float4 v_texpage"}}, false, "", UsingMSAA(),
                                UsingPerSampleShading());
      }
    }
    else
    ...
    ss << R"(
{
  ...
  v_pos.pos = float4(pos_x * pos_w, pos_y * pos_w, pos_z * pos_w, pos_w);

  //a_col0 = float4(1.0f, 1.0f, 1.0f, 1.0f); //FIXXX DD

  v_col0 = a_col0;
  #if TEXTURED
   v_tex0.x = float((a_texcoord % 65536u) * RESOLUTION_SCALE);
   v_tex0.y = float((a_texcoord / 65536u) * RESOLUTION_SCALE);

    // base_x,base_y,palette_x,palette_y
    v_texpage.x = float((a_texpage % 16u) * 64u * RESOLUTION_SCALE);
    v_texpage.y = float(((a_texpage / 16u) % 2u) * 256u * RESOLUTION_SCALE);
    v_texpage.z = float(((a_texpage / 65536u) % 64u) * 16u * RESOLUTION_SCALE);
    v_texpage.w = float(((a_texpage / 4194304u) % 512u) * RESOLUTION_SCALE);
...
{
  #if DITHERING_SCALED
    uint2 fc = coord % uint2(4u, 4u);
  #else
    uint2 fc = (coord / uint2(RESOLUTION_SCALE, RESOLUTION_SCALE)) % uint2(4u, 4u);
  #endif
  int offset = s_dither_values[fc.y * 4u + fc.x];

  #if !TRUE_COLOR
    return uint3(clamp((int3(icol) + int3(offset, offset, offset)) / 8, 0, 31));
  #else
...
uint2 ApplyTextureWindow(int2 coords)
{
  int x = (coords.x % int(u_texture_window_and.x + 1)) + int(u_texture_window_or.x);
  int y = (coords.y % int(u_texture_window_and.y + 1)) + int(u_texture_window_or.y);
  return uint2(uint(abs(x)), uint(abs(y)));
}
...
uint2 FloatToIntegerCoords(float2 coords)
{
  ...
  float2 roundedOrFlooredCoords = (RESOLUTION_SCALE == 1u) ? roundEven(coords) : floor(coords);
  float2 clampedCoords = max(roundedOrFlooredCoords, float2(0.0, 0.0));
  return uint2(clampedCoords);
}
...
float4 SampleFromVRAM(uint4 texpage, float2 coords)
{
    ...
    uint vram_value = abs(RGBA8ToRGBA5551(texel));

    // apply palette
    #if PALETTE_4_BIT
      uint subpixel = icoord.x % 4;
      uint palette_index = (vram_value / uint(pow(16.0, subpixel))) % 16u;
    #elif PALETTE_8_BIT
      uint subpixel = icoord.x % 2;
      uint palette_index = (vram_value / uint(pow(256.0, subpixel))) % 256u;
    #endif
  ...
  if (textured)
  {
    ...
    if (isLevel93)
    {
      ...
      DeclareFragmentEntryPoint(ss, 1, 1,
                                {{"", "float4 v_texpage"}, {"nointerpolation", "float4 v_uv_limits"}},
                                true, use_dual_source ? 2 : 1, !m_pgxp_depth, UsingMSAA(), UsingPerSampleShading());
      }
      else
      {
      DeclareFragmentEntryPoint(ss, 1, 1, {{"", "float4 v_texpage"}}, true, use_dual_source ? 2 : 1,
    ...
    ss << R"(
{
  ...
  uint3 vertcol = uint3(max(v_col0.rgb * float3(255.0, 255.0, 255.0), float3(0, 0, 0)));
  ...
  #if INTERLACING
   if ((fixYCoord(uint(abs(v_pos.pos.y))) % 2u) == u_interlaced_displayed_field)
     discard;
  #endif
    ...
    #else
      #if UV_LIMITS
        texcol = SampleFromVRAM(abs(v_texpage), clamp(coords, uv_limits.xy, uv_limits.zw));
      #else
        texcol = SampleFromVRAM(abs(v_texpage), coords);
      #endif
    ...
    #if !TRUE_COLOR
      icolor = uint3(max(texcol.rgb * float3(255.0, 255.0, 255.0), float3(0, 0, 0))) / 8;
      #if !RAW_TEXTURE
        icolor = (icolor * uint3(max(vertcol, float3(0, 0, 0)))) / 16;
        #if DITHERING
          icolor = ApplyDithering(uint2(abs(v_pos.pos.xy)), icolor);
        #else
          icolor = min(icolor / 8, uint3(31u, 31u, 31u));
        #endif
      #endif
    #else
  ...
  //VertexOut output;
  //output.pos = v_pos.pos;
  //output.o_col0 = float4(1.0f, 1.0f, 1.0f, 1.0f); // For example, a white color // FIXXXX DD
  //return output;
}
)";
  }
...
float4 LoadVRAM(int2 coords)
{
  ...
  return value;
#else
  //float data = samp0.Load(int3(coords.x, coords.y, 0));
  return samp0.Sample(samp0_ss, coords);
  //return LOAD_TEXTURE(samp0, coords, 0);
  //return float4(data, data, data, 1); //FIXXXX
  //return float4(0, 0, 0, 1); // DD
#endif
}
...
float3 SampleVRAM24(uint2 icoords)
{
  ...
  uint s0 = RGBA8ToRGBA5551(LoadVRAM(int2((int2(vram_coords) % int2(clamp_size)) * int(RESOLUTION_SCALE))));
  uint s1 = RGBA8ToRGBA5551(LoadVRAM(int2(((int2(vram_coords) + int2(1, 0)) % int2(clamp_size)) * int(RESOLUTION_SCALE))));
    
  // select which part of the combined 16-bit texels we are currently shading
  uint s1s0 = ((s1 * 65536) + s0) / (pow(2, (icoords.x % 2) * 8));
    
  // extract components and normalize
  return float3(
    float(s1s0 % 256) / 255.0, 
    float((s1s0 / 256) % 256) / 255.0, 
...
float3 SampleVRAM24Smoothed(uint2 icoords)
{
  ...
  uint2 low = uint2(max(int2(floor(base.x / 2) * 2, floor(base.y / 2) * 2), int2(0, 0)));
  uint2 high = low + 2u;
  float2 coeff = vec2(base.x % 2, base.y % 2) * 0.5 + 0.25;
...
float3 SampleVRAM24Smoothed(uint2 icoords)
{
  ...
  if (isLevel93)
  {
    ss << R"(
{
  uint2 icoords = uint2(abs(v_pos.pos.xy)) + uint2(abs(u_crop_left), 0u);

  #if INTERLACED
    if ((fixYCoord(icoords.y) % 2) != u_field_offset)
      discard;

    #if !INTERLEAVED
      icoords.y /= 2u;
    #else
      icoords.y = abs(floor(abs(icoords.y) / 2) * 2);
    #endif
  #endif
    ...
    #endif    
  #else
    o_col0 = float4(LoadVRAM(int2((int2(icoords) + int2(u_vram_offset)) % int2(VRAM_SIZE))).rgb, 1.0);
  #endif
  //VertexOut output;
  //output.pos = v_pos.pos;
  //output.o_col0 = float4(1.0f, 1.0f, 1.0f, 1.0f); // For example, a white color // FIXXXX DD
...
uint SampleVRAM(uint2 coords)
{
  ...
  if (isLevel93)
  {
    ss << R"(
{
   int2 sample_coords = int2(int(v_pos.pos.x) * 2, int(v_pos.pos.y));

  sample_coords += int2(u_base_coords);

  // We're encoding as 32-bit, so the output width is halved and we pack two 16-bit pixels in one 32-bit pixel.
  int left = SampleVRAM(uint2(abs(sample_coords)));
  int right = SampleVRAM(uint2(abs(sample_coords.x) + 1u, abs(sample_coords.y)));
...
std::string GPU_HW_ShaderGen::GenerateVRAMWriteFragmentShader(bool use_ssbo)
{
  ...
  if (use_ssbo && m_glsl)
  {
    ...
    if (isLevel93)
    {
      ss << "#define GET_VALUE(buffer_offset) ((buffer_offset) % 2u == 0 ? (ssbo_data[(buffer_offset) / 2u] / 65536) : (ssbo_data[(buffer_offset) / 2u] % 65536))\n\n";
    }
    else
    {
  ...
  else
  {
    if (isLevel93)
    {
      //DeclareTextureBuffer(ss, "samp0", 0, true, false);
      //ss << "#define GET_VALUE(buffer_offset) (LOAD_TEXTURE_BUFFER(samp0, int(buffer_offset)).r)\n\n";
      ss << "Texture2D<float4> samp0 : register(t0);"
            " SamplerState samp0_ss : register(s0);"
            "int GET_VALUE(int buffer_offset) {"
            "int x = buffer_offset % 16;"
            "int y = buffer_offset / 16;"
            "return samp0.Sample(samp0_ss, float2(x, y)).r;"
            "}"
            "\n\n ";
    }
    else
  ...
  if (isLevel93)
  {
    ...
    ss << R"(
{
  int2 coords2 = int2(int(v_pos.pos.x) / int(RESOLUTION_SCALE), int(fixYCoord(abs(v_pos.pos.y))) / int(RESOLUTION_SCALE));
  uint2 coords = uint2(abs(coords2));
  ...
  int2 offset2;
  offset2.x = (coords2.x < int(u_base_coords.x)) ? ((int(VRAM_SIZE.x) / int(RESOLUTION_SCALE)) - int(u_base_coords.x) + coords2.x) : (coords2.x - int(u_base_coords.x));
  offset2.y = (coords2.y < int(u_base_coords.y)) ? ((int(VRAM_SIZE.y) / int(RESOLUTION_SCALE)) - int(u_base_coords.y) + coords2.y) : (coords2.y - int(u_base_coords.y));
  uint2 offset = uint2(abs(offset2));

  uint buffer_offset = u_buffer_base_offset + (offset.y * u_size.x) + offset.x;
  int val1 = int(GET_VALUE(abs(buffer_offset))); //FIXXXX
  //int val1 = 0; // DD
  //int value =  val1 | int(u_mask_or_bits); //FIXXXX
  int value = bitwiseOrEmulated(val1, int(u_mask_or_bits));
  //int value = int(u_mask_or_bits); // DD
...
std::string GPU_HW_ShaderGen::GenerateVRAMCopyFragmentShader()
{
  ...
  if (isLevel93)
  {
    ss << R"(
{
  int2 dst_coords2 = int2(v_pos.pos.xy);
  uint2 dst_coords = uint2(abs(dst_coords2));
  ...
  int2 offset2;
  uint2 offset;
  offset2.x = (dst_coords2.x < int(u_dst_coords.x)) ? (int(VRAM_SIZE.x) - int(u_dst_coords.x) + dst_coords2.x) : (dst_coords2.x - int(u_dst_coords.x));
  offset2.y = (dst_coords2.y < int(u_dst_coords.y)) ? (int(VRAM_SIZE.y) - int(u_dst_coords.y) + dst_coords2.y) : (dst_coords2.y - int(u_dst_coords.y));
  offset = uint2(abs(offset2));

  // find the source coordinates to copy from
  int2 src_coords2 = (int2(u_src_coords) + int2(offset)) % int2(VRAM_SIZE);
  uint2 src_coords = uint2(abs(src_coords2));

  // sample and apply mask bit
#if MSAA_COPY
  float4 color = LOAD_TEXTURE_MS(samp0, int2(src_coords), f_sample_index);
#else
  float4 color = samp0.Sample(samp0_ss, src_coords);
  //float4 color = LOAD_TEXTURE(samp0, int2(src_coords), 0); //FIXXXX
  //float4 color = float4(0,0,0,1); // DD
#endif
    ...
    ss << R"(
{
  ...
  uint2 dst_coords = uint2(uint(abs(v_pos.pos.x)), fixYCoord(uint(abs(v_pos.pos.y))));
#endif

#if INTERLACED
  if ((dst_coords.y % 2u) == u_interlaced_displayed_field)
    discard;
#endif
...
#if MULTISAMPLING
  o_depth = LOAD_TEXTURE_MS(samp0, int2(v_pos.pos.xy), f_sample_index).a;
#else
  //o_depth = LOAD_TEXTURE(samp0, int2(v_pos.pos.xy), 0).a; //FIXXX
  o_depth = samp0.Sample(samp0_ss, int2(v_pos.pos.xy)).a;
  //o_depth = 0; // DD
#endif


  BREAK_HERE_FIX_LATER
...
std::string GPU_HW_ShaderGen::GenerateAdaptiveDownsampleCompositeFragmentShader()
{
  ...
  if (isLevel93)
  {
   ss << R"(
{
  float2 uv = v_pos.pos.xy * RCP_VRAM_SIZE;
   ...
   ss << R"(
{
  ...
  uint2 base_coords = uint2(v_pos.pos.xy) * uint2(RESOLUTION_SCALE, RESOLUTION_SCALE);
  for (uint offset_x = 0u; offset_x < RESOLUTION_SCALE; offset_x++)
  {
    for (uint offset_y = 0u; offset_y < RESOLUTION_SCALE; offset_y++)
