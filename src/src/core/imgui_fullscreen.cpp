...
extern void BrowseFolder();
extern int browseFolderCaller;
std::string s_file_selector_current_directory;
extern std::string s_file_selector_browse_directory;
extern bool dialogRecalled;
...
bool UpdateLayoutScale()
{
  ...
  if (isLandscape)
  {
    ...
    if (isLevel93)
    {
      ...
      else if (screenScale == 450)
      {
        fixFactor = 0.75f;
      }
      else if (screenScale == 400)
      {
        fixFactor = 0.9f;
      }
      g_layout_scale = (screen_height / (LAYOUT_SCREEN_HEIGHT)) * fixFactor;
    }
    else
    {
  ...
  else
  {
    ...
    if (isLevel93)
    {
      ...
      g_layout_scale = (screen_width / (LAYOUT_SCREEN_WIDTH)) * fixFactor;
    }
    else
    {
      g_layout_scale = screen_width / LAYOUT_SCREEN_WIDTH;
                  ...
                  ImFont* summary_font)
{
  ImRect bb;
  bool visible, hovered;
  bool pressed = MenuButtonFrame(title, enabled, height, &visible, &hovered, &bb, ImGuiButtonFlags_PressedOnRelease);
  if (!visible)
    return false;
                 ...
                 ImFont* font /*= g_large_font*/, ImFont* summary_font /*= g_medium_font*/)
{
  ...
  ImGui::SetNextWindowSize(LayoutScale(400.0f, 150.0f));

  if (ImGui::BeginPopupModal(title, nullptr, ImGuiWindowFlags_NoResize))
  {
    ImGui::SetNextItemWidth(LayoutScale(400.0f));
...
static void PopulateFileSelectorItems()
{
  ...
  {
    // s_file_selector_items.emplace_back(ICON_FA_FOLDER_PLUS "  <Browse>", std::move("#browse"), false);
    for (std::string& root_path : FileSystem::GetRootDirectoryList())
    {
      auto title = GetPreviewPath(root_path);
      s_file_selector_items.emplace_back(StringUtil::StdStringFromFormat(ICON_FA_FOLDER "  %s", title.c_str()),
  ...
  else
  {
    ...
    bool state = FileSystem::FindFiles(s_file_selector_current_directory.c_str(), "*",
    ...
    // if (!state)
    {
      ...
      s_file_selector_items.emplace_back(ICON_FA_ARROW_UP "  <Goback>", std::move(parent_path), false);
      s_file_selector_items.emplace_back(ICON_FA_STAR "  <Root>", std::move(""), false);
                      ...
                      FileSelectorFilters filters, std::string initial_directory)
{
  ...
  if (dialogRecalled)
  {
    initial_directory = s_file_selector_browse_directory;
    s_file_selector_browse_directory = "";
  }
  dialogRecalled = false;
...
void DrawFileSelector()
{
                             ...
                             ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove))
  {
    ...
    if (!s_file_selector_current_directory.empty())
    {
      auto title = GetPreviewPath(s_file_selector_current_directory);
      MenuButton(TinyString::FromFormat(ICON_FA_FOLDER_OPEN "  %s", title.c_str()), nullptr, false,
                 LAYOUT_MENU_BUTTON_HEIGHT_NO_SUMMARY);
    }

    if (s_file_selector_current_directory.empty())
    {
      if (MenuButton(ICON_FA_FOLDER_PLUS "  <Browse>", nullptr, true, LAYOUT_MENU_BUTTON_HEIGHT_NO_SUMMARY))
      {
        BrowseFolder();
      }
    }
