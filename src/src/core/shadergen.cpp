...
void ShaderGen::WriteHeader(std::stringstream& ss)
{
  ...
  else
  {
    ...
    ss << "#define LOAD_TEXTURE2(name, coords, mip) name.Load(int3(coords, mip))\n";
    ...
    ss << "#define END_ARRAY }\n";
    ss << "int bitwiseOrEmulated(int a, int b) {"
          "int result = 0;"
          "int base = 1;"
          "for (int i = 0; i < 8; i++)"
          "{"
  ...
  bool declare_sample_id /* = false */)
{
  ...
  else
  {
    ...
    if (isLevel93)
    {
      ...
      if (!depth_output || num_color_outputs == 0)
      {
        ss << "struct VertexOut{\n";
      }
      if (!depth_output)
...
std::string ShaderGen::GenerateScreenQuadVertexShader()
{
  ...
  if (isLevel93)
  {
    ss << R"(
{
  float2 v_id = input.pos;
  v_tex0 = float2(fmod(v_id.x, 2.0f) * 2.0f, fmod(v_id.y, 4.0f) >= 2.0f ? 1.0f : 0.0f);
  v_pos.pos = float4(v_tex0 * float2(2.0f, -2.0f) + float2(-1.0f, 1.0f), 0.0f, 1.0f);
   VertexOut output;
