  ...
  for (int n = 0; n < draw_data->CmdListsCount; n++)
  {
    ...
    for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
    {
      ...
      else
      {
        ...
        float xxm = clip_max.x, yym = clip_max.y, xm = clip_min.x, ym = clip_min.y;
        if (isLevel93)
        {
        ...
        const D3D11_RECT r = {(LONG)xm, (LONG)ym, (LONG)xxm, (LONG)yym};
...
void ImGui_ImplDX11_CreateFontsTexture()
{
 ...
 /* if (width > D3D_FL9_3_REQ_TEXTURE2D_U_OR_V_DIMENSION)
  {
    width = D3D_FL9_3_REQ_TEXTURE2D_U_OR_V_DIMENSION;
  }
  if (height > D3D_FL9_3_REQ_TEXTURE2D_U_OR_V_DIMENSION)
  {
    height = D3D_FL9_3_REQ_TEXTURE2D_U_OR_V_DIMENSION;
  }*/
  ...
  if (!bd->pFontSampler)
  {
    ...
    if (isLevel93)
    {
      ...
      hr = bd->pd3dDevice->CreateSamplerState(&desc, &bd->pFontSampler);
    }
    else
    {
    ...
    if (FAILED(hr))
    {
      Log_ErrorPrintf("CreateSamplerState failed: 0x%08X", hr);
    }
  }
}
...
bool ImGui_ImplDX11_CreateDeviceObjects()
{
  ...
  // Create the vertex shader
  {
    ...
    if (FAILED(D3DCompile(vertexShader, strlen(vertexShader), NULL, NULL, NULL, "main",
                          isLevel93 ? "vs_4_0_level_9_1" : "vs_4_0", 0, 0, &vertexShaderBlob, NULL)))
  ...
  // Create the pixel shader
  {
    ...
    if (FAILED(D3DCompile(pixelShader, strlen(pixelShader), NULL, NULL, NULL, "main",
                          isLevel93 ? "ps_4_0_level_9_1" : "ps_4_0", 0, 0, &pixelShaderBlob, NULL)))
      return false; // NB: Pass ID3DBlob* pErrorBlob to D3DCompile() to get error showing in (const
                    // char*)pErrorBlob->GetBufferPointer(). Make sure to Release() the blob!
